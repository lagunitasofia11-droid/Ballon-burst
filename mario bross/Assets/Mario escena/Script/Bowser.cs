...
public class Bowser : MonoBehaviour
{
    ...
    [Range(0.1f, 5f)] public float velocidadAn = 1f;         
    [Range(0.1f, 5f)] public float velocidadPumping = 1f;   
    ...
    void Start()
    {
        ...
        if (sonidoAn != null)
        {
            sonidoAn.playOnAwake = false;
            sonidoAn.Stop();
        }

        if (sonidoPumping != null)
        {
            sonidoPumping.playOnAwake = false;
            sonidoPumping.Stop();
        }
    ...
    void Update()
    {
        ...
        if (!Input.GetKey(KeyCode.L) && fillProgress > 0f)
        {
